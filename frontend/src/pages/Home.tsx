...
const Home = () => {
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden">
      ...
      <section className="py-16 md:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-navy font-serif">{t('aboutTitle')}</h2>
              ...
              <div className="grid grid-cols-2 gap-6 mb-8 text-center">
                {[
                  ...
                  <div key={idx}>
                    <h3 className="text-2xl sm:text-3xl font-bold text-gold">{item.num}</h3>
      ...
      {/* ✅ Updated Featured Rooms Section (Fetch from MongoDB) */} <section className="py-20 bg-cream"> <div className="max-w-7xl mx-auto px-4"> <div className="text-center mb-16"> <h2 className="text-4xl font-bold mb-4 text-navy font-serif">{t('featuredRooms')}</h2> <p className="text-lg text-gray-700 max-w-2xl mx-auto"> Discover our carefully curated selection of luxury accommodations </p> </div> {loading ? ( <p className="text-center text-gray-600 text-lg">Loading rooms...</p> ) : ( <div className="grid md:grid-cols-3 gap-8"> {featuredRooms.map((room) => ( <div key={room._id} className="bg-white rounded-lg shadow-lg overflow-hidden group hover:shadow-xl transition-shadow duration-300" > <div className="relative overflow-hidden"> <img src={room.image} alt={room.name} className="w-full h-64 object-cover group-hover:scale-110 transition-transform duration-500" /> <div className="absolute top-4 right-4 bg-gold text-white px-3 py-1 font-semibold rounded"> ${room.price}/night </div> </div> <div className="p-6"> <h3 className="text-xl font-semibold mb-3 text-navy ">{room.name}</h3> <div className="flex flex-wrap gap-2 mb-4"> {room.features?.map((feature, index) => ( <span key={index} className="text-sm px-3 py-1 text-gray-600 rounded-full flex items-center space-x-1" > {iconMap[feature.icon] || <Star size={16} />} <span>{feature.name}</span> </span> ))} </div> </div> </div> ))} </div> )} <div className="text-center mt-12"> <button className="bg-navy text-white px-8 py-3 rounded hover:bg-opacity-90 transition-all duration-300 font-medium inline-flex items-center space-x-2" onClick={() => { window.scrollTo({ top: 0, behavior: 'auto' }); setTimeout(() => { window.location.href = '/rooms'; }, 500); }} > <span>{t('viewAll')}</span> <ArrowRight size={18} /> </button> </div> </div> </section>

      {/* 💼 Services */}
      <section className="py-16 md:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-navy font-serif">{t('ourServices')}</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {services.map((service, index) => (
              ...
              >
                ...
                <h3 className="text-lg sm:text-xl font-semibold mb-2 text-navy">{service.title}</h3>
      ...
      <section className="py-16 md:py-20 bg-gold text-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="text-3xl sm:text-4xl font-bold mb-4 font-serif">What Our Guests Say</h2>
          <p className="text-base sm:text-xl text-white/90 mb-10">Testimonials from our valued guests</p>

          <div className="bg-navy/90 backdrop-blur-sm rounded-lg p-6 sm:p-10">
            <div className="flex justify-center mb-4">
              {[...Array(5)].map((_, i) => (
                <Star key={i} fill="currentColor" className="text-gold w-5 h-5 sm:w-6 sm:h-6" />
              ))}
            </div>
            <blockquote className="text-lg sm:text-xl italic mb-6">
              "Absolutely exceptional service and breathtaking views. A truly luxurious experience that exceeded all expectations."
            </blockquote>
            <div className="flex items-center justify-center space-x-3">
              ...
              <div>
                ...
                <p className="text-white/70 text-sm sm:text-base">New York, USA 🇺🇸</p>
              </div>
            </div>
          </div>
